...
class JudgmentAnalyzer:
    def __init__(self):
        ...
        # Original single prompt for all analyses
...
"""
    ...
    def extract_case_signature(self, rtf_content: str) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Jesteś ekspertem od prawa podatkowego. Analizujesz orzeczenia sądów administracyjnych w Polsce i tworzysz szczegółowe biuletyny analityczne."
                    },
                    {
                        "role": "user",
                        "content": full_prompt
    ...
    def analyze_multiple_judgments(self, judgments: List[Dict], progress_callback=None) -> List[Dict]:
        ...
        for i, judgment in enumerate(judgments, 1):
            ...
            case_info = judgment.get('metadata', {})
    ...
    def batch_analyze_with_limits(self, judgments: List[Dict], max_tokens_per_batch: int = 50000, progress_callback=None) -> List[Dict]:
        """
        Analyze judgments in batches to respect API limits and manage costs.
        
        Args:
            judgments: List of dictionaries containing RTF content and metadata
            max_tokens_per_batch: Maximum tokens to process in one batch
            progress_callback: Optional callback function for progress updates
        ...
        self.logger.info(f"Starting batch analysis of {len(judgments)} judgments")
        
        for i, judgment in enumerate(judgments):
            # Estimate tokens for this judgment (rough calculation)
            content_length = len(judgment.get('content', ''))
            estimated_judgment_tokens = content_length // 4  # Rough token estimation
            
            # Check if adding this judgment would exceed batch limit
            if estimated_tokens + estimated_judgment_tokens > max_tokens_per_batch and current_batch:
                # Process current batch
                self.logger.info(f"Processing batch of {len(current_batch)} judgments")
                batch_results = self.analyze_multiple_judgments(current_batch, progress_callback)
            ...
            # Add judgment to current batch
            current_batch.append(judgment)
            estimated_tokens += estimated_judgment_tokens
        
        # Process final batch
        if current_batch:
            self.logger.info(f"Processing final batch of {len(current_batch)} judgments")
            batch_results = self.analyze_multiple_judgments(current_batch, progress_callback)
            all_results.extend(batch_results)
        
        successful_count = sum(1 for r in all_results if r['success'])
        self.logger.info(f"Batch analysis complete. {successful_count}/{len(judgments)} successful")
        
        return all_results

    def get_analysis_statistics(self, results: List[Dict]) -> Dict:
        """
        ...
        # Estimate cost (rough calculation based on GPT-4 pricing)
        estimated_cost_usd = (total_tokens / 1000) * 0.03  # $0.03 per 1K tokens (rough estimate)
        ...
        return {
            ...
            'average_tokens_per_analysis': round(total_tokens / successful_analyses, 2) if successful_analyses > 0 else 0
        }
