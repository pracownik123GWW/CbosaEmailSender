...
def test_date_filter_manager():
    """Test the date filter manager functionality"""
    print("🧪 Testing DateFilterManager")
    print("=" * 50)
    
    manager = DateFilterManager()
    
    # Test date validation
    test_dates = [
        ("2025-07-25", True),
        ("2025-13-01", False),  # Invalid month
        ("25-07-2025", False),  # Wrong format
        ("2025-07-32", False),  # Invalid day
    ...
    for date_str, should_pass in test_dates:
        try:
            result = manager.validate_date_string(date_str)
            status = "✅ PASS" if should_pass else "❌ SHOULD FAIL"
            print(f"  {date_str:<12} -> {result} ({status})")
        except ValueError as e:
            status = "✅ CORRECTLY FAILED" if not should_pass else "❌ UNEXPECTED FAIL"
            print(f"  {date_str:<12} -> ERROR: {e} ({status})")
    ...
    for sig in test_signatures:
        year = manager.extract_case_date_from_signature(sig)
        print(f"  {sig:<20} -> {year}")
    
    # Test case filtering
    print(f"\n🎯 Case filtering test:")
    test_cases = [
        {"signature": "I SA/Go 33/24"},
        {"signature": "III FSK 726/24"},
        {"signature": "I SA/Gd 265/25"},
        {"signature": "Old case 123/99"},
    ]
    
    filtered, stats = manager.filter_cases_by_date(
        test_cases, 
        date(2024, 6, 1), 
        date(2025, 7, 31)
    )
    
    print(f"  Input: {len(test_cases)} cases")
    print(f"  Output: {len(filtered)} cases")
    print(f"  Stats: {stats}")
